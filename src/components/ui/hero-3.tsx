...
export const AnimatedMarqueeHero: React.FC<AnimatedMarqueeHeroProps> = ({
  ...
  return (
    ...
    >
      <div className="z-10 flex flex-col items-center pb-32 md:pb-48">
        {tagline && (
          <motion.div
            initial="hidden"
            animate="show"
        ...
        <motion.h1
          ...
          className="text-4xl md:text-7xl font-bold tracking-tighter text-foreground text-balance max-w-4xl mx-auto"
        >
          {typeof title === "string" ? (
        ...
        <motion.p
          ...
          className="mt-6 max-w-xl text-lg text-muted-foreground text-balance mx-auto"
        >
          {description}
        </motion.p>
      ...
      <div className="absolute bottom-0 left-0 w-full h-1/3 md:h-2/5 [mask-image:linear-gradient(to_bottom,transparent,black_20%,black_80%,transparent)]">
        ...
        >
          {duplicatedImages.map((src, index) => (
            <div
              key={index}
              className="relative aspect-[3/4] h-36 md:h-64 flex-shrink-0"
              style={{
